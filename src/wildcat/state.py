...
class StateDB:
    ...
    def get_last_job_metrics(
        ...
        Reads from the metrics_json column (pre-parsed at job completion).
        Falls back to parsing stdout for jobs created before the column existed.
        """
        if stage:
            row = self.conn.execute(
